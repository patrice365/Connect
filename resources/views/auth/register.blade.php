...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
        body { background: #050505; color: white; display: flex; justify-content: center; align-items: center; min-height: 100vh; position: relative; overflow-y: auto; padding: 20px; }
        ...
        .auth-card { background: rgba(15,15,15,0.7); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.1); padding: 50px 40px; border-radius: 30px; width: 100%; max-width: 460px; text-align: center; box-shadow: 0 0 40px rgba(0,0,0,0.6); margin: 20px 0; animation: fadeIn 0.8s ease-out; }
        ...
        input { width: 100%; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); padding: 16px; border-radius: 15px; color: white; outline: none; transition: 0.4s; }
        input:focus { border-color: #0ea5e9; background: rgba(14,165,233,0.05); box-shadow: 0 0 20px rgba(14,165,233,0.2); }
        .btn-auth { width: 100%; background: linear-gradient(135deg, #0ea5e9 0%, #2563eb 100%); color: white; padding: 18px; border: none; border-radius: 15px; font-weight: 800; font-size: 16px; cursor: pointer; transition: 0.3s; margin-top: 15px; text-transform: uppercase; letter-spacing: 1px; opacity: 0.5; pointer-events: none; }
        .btn-auth.enabled { opacity: 1; pointer-events: auto; }
        .btn-auth:hover.enabled { transform: translateY(-3px) scale(1.02); box-shadow: 0 15px 35px rgba(14,165,233,0.6); }
        ...
        .social-divider { display: flex; align-items: center; gap: 15px; margin-bottom: 15px; }
        ...
        .recaptcha-container { margin: 20px 0 10px 0; text-align: left; }
    </style>
</head>
<body>
    ...
    <div class="auth-card">
        ...
        <form id="register-form" action="{{ route('register') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g. Alex Rivera" required>
            </div>
            <div class="form-group">
                <label for="username">Username</label>
                <input type="text" name="username" id="username" value="{{ old('username') }}" placeholder="alexrivera" required>
            </div>
            <div class="form-group">
                <label for="email">Email Address</label>
                <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="alex@connect.com" required>
            </div>
            <div class="form-group">
            ...
            <div class="recaptcha-container">
                <div class="g-recaptcha" data-sitekey="{{ config('services.recaptcha.key') ?? env('RECAPTCHA_SITE_KEY') }}"
                     data-callback="recaptchaCallback" data-expired-callback="recaptchaExpired"></div>
                @error('g-recaptcha-response') <p style="color: #fca5a5; font-size: 13px; margin-top: 5px;">{{ $message }}</p> @enderror
            </div>
        ...
        <div class="social-buttons">
            ...
            <div style="display: flex; gap: 15px;">
                <a href="{{ route('social.redirect', ['provider' => 'youtube', 'mode' => 'register']) }}" class="social-btn youtube">
