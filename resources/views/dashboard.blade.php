    ...
    <section class="feed" style="max-width:100%;">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            ...
            <div class="card" style="border-left: 4px solid #FF0000;">
                ...
                @if($socialStats['youtube']['connected'] ?? false)
                    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px;">
                        @if($socialStats['youtube']['channel_thumbnail'] ?? null)
                        ...
                        <span style="color: #cbd5e1; font-size: 16px;">
                            {{ $socialStats['youtube']['channel_name'] ?? 'YouTube Channel' }}
                        </span>
                    </div>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; font-size: 13px; color: #cbd5e1; margin-bottom: 20px;">
                        <div>
                            ...
                            <p style="font-size: 18px; font-weight: 700;">
                                {{ isset($socialStats['youtube']['subscribers']) ? number_format($socialStats['youtube']['subscribers']) : '—' }}
                            </p>
                        </div>
                        <div>
                            <p style="color: #64748b;">Views</p>
                            <p style="font-size: 18px; font-weight: 700;">
                                {{ isset($socialStats['youtube']['views']) ? number_format($socialStats['youtube']['views']) : '—' }}
                            </p>
                        </div>
                        <div>
                            <p style="color: #64748b;">Videos</p>
                            <p style="font-size: 18px; font-weight: 700;">
                                {{ isset($socialStats['youtube']['videos']) ? number_format($socialStats['youtube']['videos']) : '—' }}
                            </p>
                        </div>
                    </div>
                ...
                @else
                    ...
                    <a href="{{ route('social.redirect', 'youtube') }}" class="btn-new" style="background:#FF0000; padding: 12px 24px; border-radius: 12px; display: inline-block;">Connect YouTube</a>
                @endif
            </div>
            ...
            <div class="card" style="border-left: 4px solid #6e5494;">
                ...
                @if($socialStats['github']['connected'] ?? false)
                    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px;">
                        @if($socialStats['github']['avatar'] ?? null)
                            <img src="{{ $socialStats['github']['avatar'] }}" style="width: 48px; height: 48px; border-radius: 50%; object-fit: cover;">
                        @endif
                        <span style="color: #cbd5e1; font-size: 16px;">{{ $socialStats['github']['username'] ?? 'GitHub User' }}</span>
                    </div>
                    <div style="margin-bottom: 20px;">
                        <p style="color: #94a3b8;">Repos: <strong style="color: white;">{{ number_format($socialStats['github']['repos'] ?? 0) }}</strong></p>
                        <p style="color: #94a3b8;">Followers: <strong style="color: white;">{{ number_format($socialStats['github']['followers'] ?? 0) }}</strong></p>
                        @if($socialStats['github']['bio'] ?? null)
                ...
                @else
                    <p style="color: #94a3b8; margin-bottom: 10px;">Not connected</p>
                    <a href="{{ route('social.redirect', 'github') }}" class="btn-new" style="background:#6e5494; padding: 12px 24px; border-radius: 12px; display: inline-block;">Connect GitHub</a>
                @endif
            </div>
        </div>

        {{-- Recent Videos (YouTube) --}}
        @if(($socialStats['youtube']['connected'] ?? false) && !empty($socialStats['youtube']['recent']))
                ...
                @foreach($socialStats['youtube']['recent'] as $video)
                    <div class="card" style="padding:15px; background:#121212; border:1px solid #1f1f1f; border-radius:12px;">
                        ...
                        <div style="display:flex; gap:20px; font-size:13px; color:#94a3b8; border-top:1px solid #2d2d2d; padding-top:8px;">
                            ...
                            <span class="comment-trigger" data-video-id="{{ $videoId }}" style="cursor:pointer;">💬 {{ number_format($comments) }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
        @elseif($socialStats['youtube']['connected'] ?? false)
        ...
        {{-- GitHub Repositories --}}
        @if(($socialStats['github']['connected'] ?? false) && !empty($socialStats['github']['repo_list']))
            <h2 style="margin-bottom: 20px; font-size: 22px; font-weight: 700; color: #cbd5e1;">Recent Repositories</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                @foreach($socialStats['github']['repo_list'] as $repo)
                    <div class="card" style="padding:15px; background:#121212; border:1px solid #1f1f1f; border-radius:12px;">
                        <div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 8px;">
                            <a href="{{ $repo['url'] }}" target="_blank" style="color: #0ea5e9; font-weight: 600; text-decoration: none; font-size: 15px;">
                                {{ $repo['name'] }}
                            </a>
                            @if($repo['language'])
        ...
        @elseif($socialStats['github']['connected'] ?? false)
...
{{-- Comment Modal (requires YouTube API routes) --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const commentPostBase = "{{ url('/youtube/comment') }}";
                    ...
                    .then(res => {
                        ...
                        if (res.error) {
                            if (res.error === 'unauthorized' || res.reconnect_url) {
                                ...
                                if (confirm(msg + '\n\nReconnect now?')) {
                                    window.location.href = res.reconnect_url || "{{ route('social.redirect', 'youtube') }}";
                                }
                                return;
                            }
