...
<div class="content-body" style="justify-content: center;">
    <section class="feed" style="max-width: 800px;">
        ...
        <div class="card">
            ...
            {{-- YouTube Form --}}
            <form id="youtube-form" action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                @if(!$connected['youtube'])
                    <div style="background: rgba(239,68,68,0.2); border:1px solid #ef4444; padding:12px; border-radius:10px; margin-bottom:20px;">
                        You must <a href="{{ route('social.redirect', 'youtube') }}" style="color:#0ea5e9;">connect your YouTube account</a> to publish videos.
                    </div>
                @endif
            ...
            {{-- GitHub Form (hidden by default) --}}
            <form id="github-form" action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data" style="display:none;">
                @csrf
                <input type="hidden" name="platform" value="github">

                @if(!$connected['github'])
                    <div style="background: rgba(239,68,68,0.2); border:1px solid #ef4444; padding:12px; border-radius:10px; margin-bottom:20px;">
                        You must <a href="{{ route('social.redirect', 'github') }}" style="color:#0ea5e9;">connect your GitHub account</a> to publish repositories.
                    </div>
                @endif
